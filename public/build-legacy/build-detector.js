...
(function () {
  ...
  try {
    var c = console;
    if (c.groupCollapsed) {
      c.groupCollapsed('%c ' + icon + '  GiftBox JS Build: ' + buildLabel + ' ', badgeStyle);
      ...
      c.log('%cFeatures    %c' + passCount + ' / ' + Object.keys(f).length + ' supported' + (failList.length ? ' — missing: ' + failList.join(', ') : ''), 'font-weight:bold;color:#94a3b8', failList.length ? 'color:#f87171' : 'color:#4ade80');
      if (c.table) {
        c.table(f);
      }
      c.groupEnd();
