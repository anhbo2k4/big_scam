...
function resolveSseUrl() {
  ...
  var fromGlobal = String(runtime && runtime.sseUrl || bootstrap && bootstrap.sseUrl || '').trim();
...
function installAudioLazyBootstrap() {
  var loaded = false;
  var bootAudio = function bootAudio() {
    if (loaded) return;
    loaded = true;
  ...
  var _onFirstGesture = function onFirstGesture() {
    try {
      bootAudio();
    } catch (e) {}
    var events = ['pointerdown', 'touchstart', 'keydown', 'click'];
    for (var i = 0; i < events.length; i++) {
      document.removeEventListener(events[i], _onFirstGesture, true);
    }
  };
  var events = ['pointerdown', 'touchstart', 'keydown', 'click'];
  for (var i = 0; i < events.length; i++) {
    document.addEventListener(events[i], _onFirstGesture, true);
  }
}
function startRealtimeRuntime() {
  ...
  var stateManager = stateMod.createStateManager({
    ...
    onStateChange: function onStateChange(next, prev, context, meta) {
  ...
  CORE_STATE.runtime = {
    ...
    renderer: renderer,
    sound: sound,
    lockAction: function lockAction(key, ttlMs) {
      return stateManager.acquireActionLock(key, ttlMs);
    },
    unlockAction: function unlockAction(key) {
      return stateManager.releaseActionLock(key);
    },
    isActionLocked: function isActionLocked(key) {
      return stateManager.isActionLocked(key);
    },
    getDebugInfo: function getDebugInfo() {
      return {
        state: stateManager.getDebugInfo(),
        queue: typeof queue.stats === 'function' ? queue.stats() : {
          total: queue.size()
        },
  ...
  window.__lmbPerfDebug.getInfo = function () {
    return CORE_STATE.runtime && typeof CORE_STATE.runtime.getDebugInfo === 'function' ? CORE_STATE.runtime.getDebugInfo() : null;
  };
  window.__lmbPerfDebug.lockAction = function (key, ttlMs) {
    return CORE_STATE.runtime && typeof CORE_STATE.runtime.lockAction === 'function' ? CORE_STATE.runtime.lockAction(key, ttlMs) : false;
  };
  ...
  window.__lmbPerfDebug.isActionLocked = function (key) {
    return CORE_STATE.runtime && typeof CORE_STATE.runtime.isActionLocked === 'function' ? !!CORE_STATE.runtime.isActionLocked(key) : false;
  };
...
function initCore() {
  ...
  var body = document.body;
  var shouldStart = body && body.dataset && body.dataset.modularRealtime === '1' || window.__lmbPerfRuntime && window.__lmbPerfRuntime.enableModularRealtime === true;
  if (shouldStart) {
