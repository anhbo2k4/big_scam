...
(function (window, document) {
  ...
  function detectEnv() {
    ...
    var isPWA = false;
    try { isPWA = !!(window.matchMedia && window.matchMedia('(display-mode: standalone)').matches); } catch (_) {}
  ...
  function safe(fn, fallback) {
    try { return fn(); } catch (_) { return fallback !== undefined ? fallback : undefined; }
  }
  ...
  function isPageFocused() {
    return safe(function () { return !!(document.hasFocus && document.hasFocus()); }, true);
  }
  ...
  function playSound(type) {
    safe(function () {
      ...
      if (!snd) return;
      if (type === 'action') {
        if (snd.playActionNotification) snd.playActionNotification();
        else if (snd.play) snd.play('action');
      } else {
        if (snd.playChatNotification) snd.playChatNotification();
        else if (snd.play) snd.play('chat');
      }
    });
  }
  ...
  function updateTabBadge(count, label) {
    safe(function () {
      ...
      if (!tab) return;
      if (count > 0) {
        if (tab.startTabBlink) tab.startTabBlink(count, label || 'Thông báo mới');
        else if (tab.incrementBadge) tab.incrementBadge(label);
      } else {
        if (tab.stopTabBlink) tab.stopTabBlink();
        else if (tab.resetBadge) tab.resetBadge();
      }
    });
  }
  ...
  function getCurrentPermission() {
    ...
    return safe(function () { return Notification.permission; }, 'default');
  }
  ...
  function requestPermission() {
    ...
    var perm = safe(function () { return Notification.permission; }, 'default');
    ...
    return safe(function () {
      ...
      var p = (result && typeof result.then === 'function')
        ? result
        : new Promise(function (resolve) {
            Notification.requestPermission(function (r) { resolve(r); });
          });
      return p.then(function (r) { state.permission = r; return r; });
  ...
  function showBrowserNotification(opts) {
    ...
    return safe(function () {
      ...
      _setTimeout(function () { safe(function () { notif.close(); }); }, 5000);
      return true;
    }, false);
  }
        ...
        .then(function (reg) {
          ...
          function activateWaiting(worker) {
            if (worker && worker.state !== 'redundant') {
              safe(function () { worker.postMessage({ type: 'SKIP_WAITING' }); });
            }
          }
    ...
    return safe(function () {
      return state.swRegistration.pushManager.getSubscription()
        .then(function (existing) {
          if (existing) { state.pushSubscription = existing; return existing; }
  ...
  function unsubscribeFromPush() {
    return safe(function () {
      ...
      return state.pushSubscription.unsubscribe()
        .then(function () {
  ...
  function _flush() {
    ...
    var title = count > 1
      ? (first.title || 'Thông báo mới') + ' (+' + (count - 1) + ' tin khác)'
      : (first.title || 'Thông báo mới');
    ...
    if (!isPageFocused()) {
      ...
      if (state.swRegistration && state.swRegistration.showNotification) {
        swPromise = safe(function () {
          return state.swRegistration.showNotification(title, {
            ...
            renotify: !!(first.tag),
            requireInteraction: false,
            data: { url: first.url || window.location.href },
          });
        }, null);
      }
      ...
      if (swPromise && typeof swPromise.then === 'function') {
        ...
        swPromise.catch(function () {
          showBrowserNotification({ title: title, body: body, icon: first.icon, tag: first.tag, url: first.url });
        });
      } else {
  ...
  function notify(options) {
    if (!options || typeof options !== 'object') {
      options = { title: String(options || 'Thông báo mới') };
    }
  ...
  function init() {
    ...
    safe(function () { window.addEventListener('focus', onFocus); });
    safe(function () {
  ...
  window.notifyNewMessage = function (options) {
    notify(typeof options === 'string' ? { title: options } : (options || {}));
  };
  ...
  window.pushNotification = {
    ...
    subscribe: function () {
      return requestPermission().then(function (perm) {
        if (perm !== 'granted') {
          return { success: false, reason: 'permission_' + perm };
        }
        var p = state.vapidPublicKey
          ? Promise.resolve(state.vapidPublicKey)
          : loadVapidKey();

        return p.then(function () {
          var regP = state.swRegistration
            ? Promise.resolve(state.swRegistration)
            : registerServiceWorker();

          return regP.then(function () {
            return subscribeToPush().then(function (sub) {
              return { success: !!sub, subscription: sub || undefined };
            });
          });
        });
      });
    ...
    unsubscribe: function () {
      return unsubscribeFromPush().then(function () { return { success: true }; });
    },
