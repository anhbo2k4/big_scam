...
(function (global) {
  ...
  const POPUP_DEFAULT_DELAY_MS = 800;

  let _instances = [];
  let _popupShownThisPage = false;

  function BannerSlider(containerEl, options = {}) {
  ...
  BannerSlider.prototype.init = async function () {
    try {
      const url = `/api/banners?location=${encodeURIComponent(this.location)}`;
    ...
    this.banners = this.banners.filter((b) => {
      const type = String(b.display_type || 'bar');
  ...
  BannerSlider.prototype._showPopupBannerIfNeeded = function () {
    ...
    const candidates = this.banners.filter((b) => {
      const type = String(b.display_type || 'bar');
    ...
    const delay = Number(banner.popup_delay_ms || POPUP_DEFAULT_DELAY_MS);
    setTimeout(() => _renderPopupBanner(banner), Math.max(0, delay));
  };
  ...
  BannerSlider.prototype._render = function () {
    ...
    this.banners.forEach((b, i) => {
      ...
      let inner = '';
      if (b.image_url) {
        inner += `<img class="bs-img" src="${_esc(b.image_url)}" alt="${_esc(b.text || '')}" loading="lazy">`;
      }
      if (b.text) {
        inner += `<span class="bs-text" ${b.text_color ? 'style="color:' + _esc(b.text_color) + '"' : ''}>${_esc(b.text)}</span>`;
      }
  ...
  BannerSlider.prototype.goTo = function (idx) {
    ...
    const slides = this._track.querySelectorAll('.bs-slide');
    const dots = this.el.querySelectorAll('.bs-dot');
  ...
  function _shouldShowPopup(banner) {
    const freq = String(banner.popup_frequency || 'session');
    const key = _popupStorageKey(banner);
    try {
  ...
  function _renderPopupBanner(banner) {
    const existing = document.getElementById('bsPopupOverlay');
    ...
    const ratio = _ratioForBanner(banner.aspect_ratio);
    const popupWidth = _safeCssSize(banner.popup_width);
    const popupHeight = _safeCssSize(banner.popup_height);
    const sizeCss = popupWidth ? ('width:' + popupWidth + ';max-width:96vw;') : 'width:min(900px,96vw);';
    const heightCss = popupHeight
      ? ('height:' + popupHeight + ';max-height:92vh;')
      : ('aspect-ratio:' + ratio + ';max-height:92vh;');
    ...
    const dismissible = banner.popup_dismissible !== false;
    if (dismissible) {
    ...
    const text = banner.text ? '<div style="margin-top:12px;font-size:16px;line-height:1.55;font-weight:600;">' + _esc(banner.text) + '</div>' : '';
    const customHtml = banner.design_html ? '<div class="bs-popup-custom" style="position:absolute;inset:0;overflow:auto;padding:14px;">' + banner.design_html + '</div>' : '';
    const customCss = banner.design_css ? '<style>' + banner.design_css + '</style>' : '';
    ...
    const autoClose = Number(banner.auto_close_ms || 0);
    if (autoClose > 0) {
      setTimeout(() => {
    ...
    if (dismissible) {
      const closeBtn = card.querySelector('button[aria-label="Dong banner"]');
      if (closeBtn) {
  ...
  function _applyPopupShowAnimation(card, animation) {
    const a = String(animation || 'zoom');
    if (a === 'fade') {
      card.animate([{ opacity: 0 }, { opacity: 1 }], { duration: 260, easing: 'ease-out' });
      return;
    }
    if (a === 'slide-up') {
      card.animate([{ opacity: 0, transform: 'translateY(38px)' }, { opacity: 1, transform: 'translateY(0)' }], { duration: 320, easing: 'cubic-bezier(.2,.8,.2,1)' });
      return;
    }
    if (a === 'slide-down') {
      card.animate([{ opacity: 0, transform: 'translateY(-38px)' }, { opacity: 1, transform: 'translateY(0)' }], { duration: 320, easing: 'cubic-bezier(.2,.8,.2,1)' });
    ...
    if (a === 'flip') {
      card.animate([{ opacity: 0, transform: 'perspective(800px) rotateX(-20deg) scale(.96)' }, { opacity: 1, transform: 'perspective(800px) rotateX(0) scale(1)' }], { duration: 360, easing: 'ease-out' });
      return;
    }
    card.animate([{ opacity: 0, transform: 'scale(.9)' }, { opacity: 1, transform: 'scale(1)' }], { duration: 280, easing: 'cubic-bezier(.2,.8,.2,1)' });
  }
  ...
  function _closePopupWithAnimation(overlay, card, animation) {
    ...
    let keyframes;
    if (a === 'zoom-out') {
      keyframes = [{ opacity: 1, transform: 'scale(1)' }, { opacity: 0, transform: 'scale(.85)' }];
    } else if (a === 'slide-down') {
      keyframes = [{ opacity: 1, transform: 'translateY(0)' }, { opacity: 0, transform: 'translateY(40px)' }];
    } else if (a === 'slide-up') {
      keyframes = [{ opacity: 1, transform: 'translateY(0)' }, { opacity: 0, transform: 'translateY(-40px)' }];
    } else {
      keyframes = [{ opacity: 1 }, { opacity: 0 }];
    }
    const ani = card.animate(keyframes, { duration: 220, easing: 'ease-in' });
    ani.onfinish = () => overlay.remove();
  }
  ...
  BannerSlider.init = function (selector, options) {
    ...
    if (!el) return null;
    const slider = new BannerSlider(el, options);
    slider.init();
  ...
  BannerSlider.initAll = function () {
    document.querySelectorAll('[data-banner-slider]').forEach((el) => {
      const slider = new BannerSlider(el);
      slider.init();
      _instances.push(slider);
    });
  };
